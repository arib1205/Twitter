...
var messagesDiv = $('#tweet-container');
messagesDiv.empty();
...
var registerSubmit = $("#register-btn");

var loggedInUser;

var tweetInput = $("#tweet-box");
var tweetBtn = $("#tweet-btn");

loginSubmit.on('click', function () {
  if (loginUsername.val() === '' || loginPassword.val() === '') alert("Enter correct credentials");else {
...
registerSubmit.on('click', function () {
  ...
  if (registerUsername.val() === '' || registerPassword.val() === '') alert("Enter correct credentials");else {
...
channel.on("logged_in", function (payload) {
...
var showMyTweets = $('#show-my-tweets');
var showRetweets = $('#show-retweets');
var follow = $('#follow');
...
followBtn.on('click', function () {
  ...
  if (follow == '') alert('Enter at least one follower');else {
    channel.push('subscribe_to', { follow: follow, username: loggedInUser });
    $('#follow-box').val('');
  }
});

channel.on("subscribed_to", function (payload) {
  alert(payload.response);
});

tweetBtn.on('click', function () {
  console.log("Tweet btn clicked");
  if (tweetInput.val() === '') alert("Enter atleast one tweet!");else {
    //alert(loggedInUser);
...
var nav = $('#nav');
channel.on("tweeted", function (payload) {
  alert(payload.response);
});

channel.on("showing_my_tweets", function (payload) {
  hideAll();
  $('#h3-tweets').css('display', 'block');
...
//show retweets
var myRetweets = $('#my-retweets');

channel.on("showing_my_retweets", function (payload) {
  hideAll();
  $('#h3-retweets').css('display', 'block');
...
channel.on("showing_news_feed", function (payload) {
...
channel.on('retweeted', function (payload) {
  alert(payload.response);
});
...
var hashBtn = $('#hash-btn');
hashBtn.on('click', function () {
  var hash = $('#hash-box').val();
  if (hash == '') alert('Enter at least one hash string to query');else {
    channel.push('query_hash', { hash: hash });
    $('#hash-box').val('');
  }
});

channel.on("queried_hash", function (payload) {
  hideAll();
  $('#h3-hash').css("display", "block");
...
var mentionBtn = $('#mention-btn');
mentionBtn.on('click', function () {
  var mention = $('#mention-box').val();
  if (mention == '') alert('Enter at least one mentioned user to query');else {
    channel.push('query_mention', { mention: mention });
    $('#mention-box').val('');
  }
});

channel.on("queried_mention", function (payload) {
  hideAll();
  $('#h3-mention').css("display", "block");
